            ...
            if (($InfoLevel.Infrastructure.ServerConfig -gt 0) -and ($ServerConfigRestAPI)) {
                ...
                Section -Style Heading3 'RESTful API' {
                    ...
                    if ($HealthCheck.Infrastructure.ServerConfig) {
                        ...
                        $ServerConfigRestAPIInfo | Where-Object { [datetime]((Get-Date).AddDays(+90)).Date.DateTime -gt $_.'Expiration Date' } | Set-Style -Style Critical -Property 'Expiration Date'
                        foreach ( $OBJ in ($ServerConfigRestAPIInfo | Where-Object { [datetime]((Get-Date).AddDays(+90)).Date.DateTime -gt $_.'Expiration Date' })) {
