...
function Get-AbrVboLog {
    <#
    .SYNOPSIS
        Collects diagnostic information for AsBuiltReport.Veeam.VB365 troubleshooting.
    .DESCRIPTION
    ...
    .EXAMPLE
        Get-AbrVboLog -OutputFolderPath 'C:\Logs' -IncludeErrorDetails -PassThru
    ...
    begin {
        Write-Verbose 'Get-AbrVboLog: Starting diagnostic collection.'
        $TimeStamp = Get-Date -Format 'yyyyMMdd_HHmmss'
        $FileName = "AbrVboDiagnostics_$TimeStamp.json"
    ...
    process {
        ...
        try {
            ...
            Write-Host "  [Get-AbrVboLog] Diagnostic bundle saved to: $OutputFile" -ForegroundColor Green
        } catch {
            Write-Warning "Get-AbrVboLog: Failed to write diagnostic file '$OutputFile': $($_.Exception.Message)"
        }
        ...
        Write-Verbose 'Get-AbrVboLog: Diagnostic collection complete.'
    }
}
